...
class Tests(object):
    ...
    def check_sampler(
        self, cutoff=None, N=None, p0=None, adapt=False, weak=False, fail=False
    ):
        if cutoff is None:
            cutoff = self.cutoff
        if N is None:
        ...
        if fail:
            ...
            try:
                ...
                assert (
                    type(fail) is not type or type(e) is fail
                ), "Sampler failed with unexpected exception type."
                return
        else:
            for p, logpost, loglike, ratios in self.sampler.sample(
                p0, iterations=N, adapt=adapt, swap_ratios=True
            ):
                assert np.all(logpost > -np.inf) and np.all(
                    loglike > -np.inf
                ...
                assert np.all(
                    np.diff(self.sampler.betas) != 0
                ), "Temperatures have coalesced."
                assert np.all(
                    np.diff(self.sampler.betas) < 0
                ), "Temperatures incorrectly ordered."

        assert np.all(self.sampler.acor > 0), "Invalid autocorrelation lengths."
